...
function real_weight(node::Node)
    ...
    if total != weights[1] * length(weights)
        ...
        bad_node = node.children[find(x -> real_weight(x) == wrong, node.children)[1]]

        target = bad_node.weight - (wrong - correct)
        println(target)
...
function array_to_nodes(data::Array)
    ...
    for d in data
        unparsed = split(d)
        name = unparsed[1]
        weight = parse(Int64, unparsed[2][2:end-1])
        if length(unparsed) >= 4
            children = [replace(x, ",", "") for x in unparsed[4:end]]
        else
            children = String[]
        end

        push!(nodes, Node(name, weight, children))
    end

    nodes
end

function build_node(nodes::Array, i::Int64)
    node = nodes[i]
    deleteat!(nodes, i)
    ...
    for child in node.children
        ...
        idx = find(x -> x.name == child, nodes)[1]
        push!(node_children, build_node(nodes, idx))
    end
...
function build_first_node(nodes::Array)
    for i in 1:length(nodes)
        if !isempty(nodes[i].children) && isa(nodes[i].children[1], String)
            push!(nodes, build_node(nodes, i))
            return
        end
    end
end

function build_nodes(nodes::Array)
    while length(nodes) > 1
        build_first_node(nodes)
    end
end
...
nodes = array_to_nodes(data)

build_nodes(nodes)
println(nodes[1].name)
real_weight(nodes[1])
