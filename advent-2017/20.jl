...
mutable struct Particle
    ...
    Particle(id::Int64, data::String) = begin
        d = split(data, ", ")
        x, y, z = parse.(split(d[1][4:end-1], ","))
        vx, vy, vz = parse.(split(d[2][4:end-1], ","))
        ax, ay, az = parse.(split(d[3][4:end-1], ","))
        new(id, x, y, z, vx, vy, vz, ax, ay, az)
    end
end

distance(p::Particle) = abs(p.x) + abs(p.y) + abs(p.z)
...
tick(ps::Array) = foreach(tick, ps)

function collide(ps::Array)
    d = Dict()
    del = Int64[]
    ...
    deleteat!(ps, del |> unique |> sort)
end

function swarm(data::Array; particles_collide::Bool = false)
    particles = [Particle(i-1, d) for (i, d) in data |> enumerate]

    zbuf_particle = nothing
    zbuf_changed = 0
    for i in Iterators.countfrom(1)
        if i - zbuf_changed > 350
            break
        end
        ...
        if min_particle != zbuf_particle
            zbuf_particle = min_particle
            zbuf_changed = i
        end
    end
...
println(swarm(data))
println(swarm(data, particles_collide=true))
