...
function execute_instruction(collector::DataStructures.DefaultDict, data::String, largest::Int64)
    d = split(data)

    qvar = collector[d[5]]
    qexpr = join(d[6:end])
    query = eval(parse("$qvar$qexpr"))
    if query
        if d[2] == "inc"
            collector[d[1]] += parse(Int, d[3])
        else
            collector[d[1]] -= parse(Int, d[3])
        end

        if collector[d[1]] > largest
            largest = collector[d[1]]
        end
    end
    return largest
end

function execute_program(datas::Array)
    registers = DataStructures.DefaultDict(0)
    largest = 0
    for data in datas
        largest = execute_instruction(registers, data, largest)
    end
...
registers, largest = execute_program(data)
println(maximum(values(registers)))
println(largest)
